...
from utils.general import (LOGGER, check_suffix, check_version, colorstr, increment_path,
                           make_divisible, non_max_suppression, scale_coords, xywh2xyxy, xyxy2xywh, exif_transpose, letterbox)
from utils.plots import Annotator, colors, save_one_box
from utils.torch_utils import copy_attr, time_sync
from torch.nn import init
from models.attention import CBAMBlock, CBAMBlock2, GAM
...
class FusionNet(nn.Module):
    def __init__(self):
        ...
        # rgb 分支
        self.conv1_rgb = ConvNoBN(3, 16, 3, 1, 1) # in_chs out_chs kernel_size stride padding
        self.grdb1_rgb = RGBD(16, 32)
        self.grdb2_rgb = RGBD(32, 48)
        # ir 分支
        self.conv1_ir = ConvNoBN(3, 16, 3, 1, 1) # in_chs out_chs kernel_size stride padding
        self.grdb1_ir = RGBD(16, 32)
        self.grdb2_ir = RGBD(32, 48)
        # fused 
        self.conv1_fused = Conv(96, 64, 3, 1)
        self.conv2_fused = Conv(64, 32, 3, 1)
        self.conv3_fused = Conv(32, 16, 3, 1)
        self.conv4_fused = nn.Sequential(
            nn.Conv2d(16, 3, 3, 1, 1),
            nn.BatchNorm2d(3),
            nn.Tanh()
        )
    ...
    def forward(self, x):
        ...
        x3 = torch.cat((x1, x2), dim=1) # 融合分支
        x3 = self.conv1_fused(x3)
        x3 = self.conv2_fused(x3)
        x3 = self.conv3_fused(x3)
        x3 = self.conv4_fused(x3)
        return x3
...
class RGBD(nn.Module):
    def __init__(self, c1, c2):
        ...
        self.convdown = nn.Conv2d(3 * c1, c2, 1, 1)
        self.sobelconv = Sobelxy(c1)
        self.convup = nn.Conv2d(c1, c2, 1, 1)
        self.relu = nn.LeakyReLU(0.1)
...
class NIN(nn.Module):
    ...
    def __init__(self, channel):
        ...
        self.conv = nn.Conv2d(self.channel, channel, 1, 1)
...
class SEAttention(nn.Module):
    ...
    def __init__(self, channel=0, reduction=16):
        super().__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.channel = channel * 2 # 设置为参数的二倍， 因为 img 需要 concat 起来
...
# challow add
class OnlyT(nn.Module):
    # use only infrared
    def __init__(self, arg):
        super(OnlyT, self).__init__()
        self.arg = arg
...
# challow add 
class OnlyV(nn.Module):
    # use only visible
    def __init__(self, arg):
        super(OnlyV, self).__init__()
        self.arg = arg
...
# challow add
class AddNone(nn.Module):
    # use only infrared
    def __init__(self, arg):
        super(AddNone, self).__init__()
        self.arg = arg
...
class STN_Net(nn.Module):
    ...
    def forward(self, x):
        x1, x2 = x[0], x[1] # x1  rgb  x2   ir   RGB 向 IR 对齐
        # x2, x1 = x[0], x[1] #   rgb  ir      IR 向 RGB 对齐 不太行
        grid_ = F.affine_grid(
            self.theta.unsqueeze(0).repeat(x2.shape[0], 1, 1),
            x2.shape, align_corners=True
        )

        grid = self.Backbone(x2).permute(0, 2, 3, 1)

        x1 = F.grid_sample(x1, grid+grid_, align_corners=False) # 这里是对  x1 进行变换操作
...
class DetectMultiBackend(nn.Module):
    ...
    def forward(self, im, augment=False, visualize=False, val=False):
        ...
        if self.pt or self.jit:  # PyTorch
            ...
            return y if val else y[0] # 为了直接val.py修改这儿
